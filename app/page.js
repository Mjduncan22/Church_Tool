...
export default function WardStudy() {
  ...
  function submitMemorize(event) {
    ...
    const expected = memorizeMember.full_name.includes(',')
      ? memorizeMember.full_name.split(',')[1].trim().split(/\s+/)[0].toLowerCase()
      : memorizeMember.full_name.trim().split(/\s+/)[0].toLowerCase();
    const isCorrect = answer === expected;
    const retryingAfterIncorrectGuess = memorizeFeedback?.startsWith('Answer:');
    setScores((current) => {
    ...
    if (isCorrect) {
      setMemorizeFeedback(null);
      setMemorizeQueue((current) => retryingAfterIncorrectGuess
        ? (current.length > 1 ? [...current.slice(1), current[0]] : current)
        : current.filter((name) => name !== memorizeMember.full_name));
      return;
    }
    setMemorizeFeedback(isCorrect ? 'Correct' : `Answer: ${expected}`);
  }
            ...
            {memorizeMember ? (
              <div className="grid overflow-hidden rounded-3xl border border-slate-800 bg-slate-900 md:grid-cols-[minmax(0,1.15fr)_minmax(20rem,0.85fr)]">
                ...
                <div className="flex flex-col justify-center p-6 md:p-10">
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-400">Type the first name</p>
                  ...
                  {memorizeFeedback && <div className="mt-5 rounded-xl border border-amber-800 bg-amber-950/40 p-4 text-sm text-amber-200"><p className="font-semibold">{memorizeFeedback}</p><p className="mt-1 text-xs">Type the first name exactly as shown above to continue.</p></div>}
